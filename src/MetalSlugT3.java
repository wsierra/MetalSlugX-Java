...
public class MetalSlugT3 extends Juego {
	...
	public Path getScreenshot() {
		return MetalSlugT.DIR_IMAGENES.resolve("screenshot.png");
	}
	...
	private class EjecucionMST extends EjecucionJuego {
		...
		protected void inicializar() {
			...
			try {
				fondo = ImageIO.read(MetalSlugT.DIR_IMAGENES.resolve("mision1.gif").toFile());
				conf = new Configuracion(MetalSlugT.DIR_CONF.resolve("conf.data"));
			}
			catch (Exception ex) {
			...
			sprite = new Sprite(DIR_SPRITES.resolve("marco.gif"), null);
		...
		protected void actualizar() {
			...
			if (isKeyPressed(KeyEvent.VK_F1)) {
						...
						lanzarDialogo("Configuracion", true, pConf, null);


						System.out.println(this);
						try {
							conf.guardar(MetalSlugT.DIR_CONF.resolve("conf.data"));
						}
						catch (Exception ex) {}
			...
			if (isKeyPressed(conf.getControl("izquierda"))) {
				if (dir == -1)
					area.x -= 4;
				else
					dir = -1;
			...
			else if (isKeyPressed( conf.getControl("derecha"))) {
				if (dir == 1)
					area.x += 4;
				else
					dir = 1;
			...
			else if (isKeyPressed( conf.getControl("saltar"))){
				cont = 1;
				saltando = true;
			}
	...
	private class Marco{
		...
		Sprite sprite = new Sprite(DIR_SPRITES.resolve("marco.gif"), null);
		int estado;
		private Rectangle area = new Rectangle(100, 311, 36, 71);
		private AdminSprite adm = new AdminSprite();
